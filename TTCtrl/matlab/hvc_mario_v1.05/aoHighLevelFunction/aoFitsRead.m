function output_image=aoFitsRead(filename,subset,scaleQ)
% aoFitsRead reads a FITS image into a Matlab variable.
%
%  data = aoFitsRead(filename);
%  data = aoFitsRead(filename,range);
%  data = aoFitsRead(filename,range,scaleQ);
%
...
%
%  Examples:
%    im=aoFitsRead('n2715_J');                        % Assumes .fits extension
%    im=aoFitsRead('n2715_lJ.fits','10:313,15:175');  % Reads a subimage
%    im=aoFitsRead('n2715_lJ.fits','full',0);         % Full image, no scaling
%
%  Known deficiences:
...
%"aoFitsReadHeader" routine which scans through the whole header block
%looking for specific keywords (a slow process). We want to
%do this here before opening the file, because the aoFitsReadHeader function
%also opens the file internally and we want to avoid mixing calls to
%"aoFitsReadHeader" with explicit reads from the data file within this
...
if scaleQ==1
   bscale=aoFitsReadHeader(filename,'BSCALE');
   bzero=aoFitsReadHeader(filename,'BZERO');
...
% attenzione a come ho modificato la patch fatta da diti... non sono certo che funzioni sempre
n_card=0;
[d,simple,d]=parse_card(setstr(fread(file,80,'uchar')')); n_card=n_card+1;
[d,bitpix,d]=parse_card(setstr(fread(file,80,'uchar')')); n_card=n_card+1;
[d,naxis,d]=parse_card(setstr(fread(file,80,'uchar')'));  n_card=n_card+1;
naxis1=1;
naxis2=1;
naxis3=1;
if naxis>=1
   [d,naxis1,d]=parse_card(setstr(fread(file,80,'uchar')')); n_card=n_card+1;
end
if naxis>=2
   [d,naxis2,d]=parse_card(setstr(fread(file,80,'uchar')')); n_card=n_card+1;
end
if naxis>=3
   [k,naxis3,d]=parse_card(setstr(fread(file,80,'uchar')')); n_card=n_card+1;
end
...
% [k,naxis3,d]=parse_card(setstr(fread(file,80,'uchar')'));
% % patch to be able to read Arcetri's fits files where naxis2 is missing
% if naxis2=='T'
%    naxis2=1;
% end
% if naxis3=='T'
...
%of the next row
pixel_row_skip = naxis2 - nc;

% move to first byte
% RBI: corrected types to be more consistent with Matlab types
if bitpix==-64
...
% RBI: corrected to generate a zero vector with proper data type
% X=zeros(nr*nc,1);
% eval(sprintf('X=%s(zeros(nr*nc,1));',type));
% for il=1:nl
%    startIndex = 1;
%    finIndex = nc;
%    for i=1:nr
...
%    output_image(:,:,il)=reshape(X,nr,nc)';
% end

% MAN: completely rewritten to read all the bin file in one shot and then reshaping the 
% read data according to the function call, done to increase the function duration
for il=1:nl
   X=fread(file,(nc+byte_column_skip)*nr,type);
   %Scale the data
   X = bscale*X + bzero;
   X = reshape(X,nr,nc+byte_column_skip);
   output_image(:,:,il)=X(:,1:nc)';
end
