...
a1=aoRead('hvc_TT2_step_ptr_preshaper_cmd',0:sys_data.nAct-1,1,sys_data.mirrorActMap);
a2=aoRead('hvc_TT2_step_ptr_preshaper_curr',0:sys_data.nAct-1,1,sys_data.mirrorActMap);

aoWrite('hvc_TT2_step_ptr_preshaper_cmd',round(sys_data.pres_buff_size/sys_data.cmdPreshTimeSet/sys_data.cntFreq)*ones(sys_data.nAct,1),0:sys_data.nAct-1,1,sys_data.mirrorActMap);
aoWrite('hvc_TT2_step_ptr_preshaper_curr',round(sys_data.pres_buff_size/sys_data.ffPreshTimeSet/sys_data.cntFreq)*ones(sys_data.nAct,1),0:sys_data.nAct-1,1,sys_data.mirrorActMap);

% set zero position
aoWrite('hvc_TT2_pos_command',[0 0 0 0],0:3);
aoWrite('hvc_TT2_curr_command',[0 0 0 0],0:3);
aoWrite('hvc_TT2_bias_current',sys_data.bias_current_XY,0:3);
pause(1);

% lettura degli strain gauge
aoBufferArray(1).bufferName='hvc_TT2_distance';	           % name of buffer, is not mandatory, just useful
...
aoBufferArray(1).dspPointer=aoGetAddress('hvc_TT2_distance'); % pointer of read/write element
...
% aoWrite('hvc_TT2_bias_command',[40e-6 90e-6],0:1);
...
aoWrite('hvc_TT2_bias_command',[x_pos y_pos 0 0],0:3);
aoWrite('hvc_TT2_control_enable',[1 1],0:1); % XXX
aoWrite('hvc_TT2_post_loop_gain',sys_data.post_loop_gain,0:3);
pause(.1);
aoWrite('hvc_TT2_pre_loop_gain',sys_data.pre_loop_gain,0:3);
pause(.5);
aoWrite('hvc_TT2_bias_command',sys_data.bias_command_XY,0:3);
pause(.5);
aoWrite('hvc_TT2_step_ptr_preshaper_cmd',a1,0:sys_data.nAct-1,1,sys_data.mirrorActMap);
aoWrite('hvc_TT2_step_ptr_preshaper_curr',a2,0:sys_data.nAct-1,1,sys_data.mirrorActMap);
