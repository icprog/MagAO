; $Id: inc_fc_bench.pro,v 1.1 2007/04/12 12:58:14 labot Exp $$
;+
;   NAME:
;    INC_FC_BENCH
;
...
;           , DISTRBANCE=disturbance, NOVERB=noverb, BENCH=bench
;
;   INPUT:
;    vect:              Slope vector.
...
;
;   KEYWORDS:
;       BENCH = working bench, if set Memory bench B for optical loop was used.
;
;
;    HISTORY
;
...
;-
;
Pro inc_fc_bench, vect, disturb_vect, WAIT=sleep_time, MIRROR=mirror, SWITCHB=is_switch, BUFSKIP=bufskip, DOCHECK=docheck, DISTURBANCE=disturbance, BENCH=bench
    ...
    if keyword_set(is_switch) then begin
        ...
        if keyword_set(bench) then pbs = 2UL^0 or 2UL^6 or 2UL^7 else pbs =  2UL^6 or 2UL^7 ;(diagnostic SDRAM + FASTLINK ENABLE)
        dummy = [1UL, 0UL]
        wfs += 1
        ...
        ;wait, 0.01
;TO DEBUG
        err = read_seq_dsp(0,rtr_map.param_selector, 1l, param, /UL)
        if err ne adsec_error.ok then message, "Error encountered: "+strtrim(err,2)
        print, 'Param block selector:', param
;END DEBUG
        ...
        if keyword_set(docheck) then begin
            ...
            if keyword_set(bench) then begin
            ;true_modes = diagonal_matrix(*rtr.g_gain_b)##(*rtr.b0_b_matrix ## transpose(vect) + delayB) + delayA 
                ;step_0 = mamano(*rtr.b0_b_matrix,transpose(vect))+ delayB
                step_0 =transpose(total([mamano(*rtr.b0_b_matrix,transpose(vect)),delayB], 1, /preserve,/Tpool_no))
                
                true_modes =transpose(total([mamano(diagonal_matrix(*rtr.g_gain_b),transpose(step_0)),delayA], 1, /preserve,/Tpool_no))
                ;true_modes = mamano(diagonal_matrix(*rtr.g_gain_b),transpose(step_0))+delayA
            endif else begin
                ;step_0 = mamano(*rtr.b0_a_matrix,transpose(vect))+ delayB
            ...
            if max(abs(dsp_modes-true_modes)) ne 0.0 then begin
              err_calc = max(abs(dsp_modes-true_modes))
              print, err_calc    
              if err_calc gt err_mac then message, "Errore calcolo modi"
            endif
        ...
        ;ff_current = adsec.ff_matrix ## transpose(ff_command)
            ...
            old_modes = dsp_modes
            if keyword_set(bench) then begin 
                true_delayb = mamano(*rtr.b_delay_b_matrix,transpose(old_vect))
                ;true_delayb = *rtr.b_delay_b_matrix ## transpose(old_vect)
        ...
        if err ne adsec_error.ok then message, "Error encountered: "+strtrim(err,2)
        if keyword_set(bench) then bb = bb or 2UL^0 or 2UL^6 else bb = bb or 2UL^6
