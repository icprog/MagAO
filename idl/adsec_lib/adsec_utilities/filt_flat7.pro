...
function filt_flat7, sys_status_flat, sys_status_ini, DISPL=disp_plot, INIT=file1, FLAT=file2, ACT_FILTERED=act_removed_list, ACT_LIST=act_list_tmp, SKIPFILT=skipfilt
...
;    idact = where(abs(ftot) gt (abs(f2_nopist.current)))
    ...
    if n_elements(act_list_tmp) gt 0 then begin
        ...
        err = complement(act_list, clact, dlist)
        if err ne 0 then log_print, "Error: wrong list passed. The result can be without any sense"
        act_removed_list = act_list
    endif

    err = complement(clact,cl1, removed_cl)
err = 1
    if err eq 0 and n_elements(removed_cl) gt 0 then begin
        ...
        ;       +string(f2_nopist.current[dlist[idact]]+ftot[dlist[idact]], FORMAT='(F5.2)')
            ...
            vvtmp = fltarr(adsec.n_actuators)
            vvtmp[removed_cl]=1
            idbad = where(vvtmp[cl1])
            vvtmp = fltarr(adsec.n_actuators)
            vvtmp[clact]=1
            idnotbad = where(vvtmp[cl1])
            ff_matrix = readfits(!AO_STATUS.ff_matrix)
            dummy = ff_matrix_reduction(ff_matrix, idbad, dx, k00, k01, k10, k11)
    ...
    if ~keyword_set(SKIPFILT) then begin
        ...
        ;       +string(f2_nopist.current[dlist[idact]]+ftot[dlist[idact]], FORMAT='(F5.2)')
            ...
            if act_removed eq 0 then begin
                ...
                if n_elements(act_list) eq 0 then act_removed_list = [act_removed_list, dlist[idact]]
            endelse
    ...
    ;        print, idbad
            ...
            fcmd[idbad] = dc0
            fcmd[idnotbad] = dc1
            if n_elements(act_list) gt 0 then return, fcmd 
