...
% aoProgramFlash(flashBlock,[filename],[firstDsp,[lastDsp]])
...
while i<fileLen
   if i+sub_len<fileLen
      mgp_op_write_flash(firstDsp,lastDsp,sub_len,startAddress+i,fileContent(i+1:i+sub_len));
   else
      mgp_op_write_flash(firstDsp,lastDsp,fileLen-i,startAddress+i,fileContent(i+1:fileLen));
   end
   i=i+sub_len;
end
%compare
flashContent=mgp_op_rdseq_flash(firstDsp,firstDsp,fileLen,startAddress);
