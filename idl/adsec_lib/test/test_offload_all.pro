;$Id: test_offload_all.pro,v 1.1 2007/04/12 13:56:37 labot Exp $$
;+
;   NAME:
;       TEST_OFFLOAD_ALL
;
;   PURPOSE:
;    ROUTINE TEST FOR THE USE OF OFFLOAD COMMANDS VECTOR AND OFFLOAD FORCES VECTOR.
;
;   NOTE:
;
;   USAGE:
...
;   04  April 2007, Daniela Zanotti(DZ)
;       added the test to  all the cmd and force vector
;-

Pro test_offload_all, CMD = cmd, FORCE=force, WFS_MODE=wfs_mode
    ...
    for i=0, ntime do begin
       
        vect = fltarr(adsec.n_actuators)
        val = 0.1 * randomn(seed, adsec.n_actuators)
        ;vect[i] = val
        vect = val
        ...
        if is_cmd then begin
            ...
            if total(curr_buf_c-vect) ne 0.0 then begin
            ;id = where(curr_buf_c ne 0, cc)
            id = where((curr_buf_c eq vect) ne 1, cc)
            ...
            if total(curr_buf_f-vect) ne 0.0 then begin
            ;if (cc ne 1) or (id[0] ne i) then begin
            id = where((curr_buf_f eq vect) ne 1, cc)
                  print, "################ Time #"+string(i, format='(I3.3)')
                  print, "################ Unmatches: ", strtrim(id,2)
            endif
