function build_int_mat, tracknum, ACT_SAMPLING=act_sampling, N2REMOVE=zern2remove, xyre=xyre
    ...
    for i=0, nfiles-1 do begin
        ...
        print, 'restoring file'+file2read[i]
                if (i eq 0) then begin
                    ...
                    if keyword_set(xyre) then begin 
                        pupil_xyre = xyre
                    endif else begin
...
command_matrix = readfits( adsec_path.meas+'if_functions/'+tracknum[0]+'/command_matrix.fits')
print, ff
    save,  filename=ff,data $
        ,  cl_act, n_modes, n_pix_x, n_pix_y, n_meta_pix_x, n_meta_pix_y $
        , x0,x1,y0,y1,pupil_xyre, mask_cx, mask_cy, min_pix, intersect_mask_idx, max_rms, size_ini, data_ver, adsec_save, tracknum, command_matrix
