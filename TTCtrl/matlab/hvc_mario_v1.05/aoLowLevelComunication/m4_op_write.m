function m4_op_write(firstFin,lastFin,len,startAddress,data,varargin)
% m4_op_write(firstFin,lastFin,len,startAddress,data,[connectionNr],[dataType],[speedTrans],[flag])
...
% buf(10) -- 0x09 rd/wr  of fl remote data length  (in bytes)
% buf(11) -- 0x0A rdonly of fins number
% buf(12) -- 0x0B rd/wr  of local start addr       (in bytes - absolute value see NIOS memory map)
% buf(13) -- 0x0C rd/wr  of local data length      (in bytes)
% buf(14) -- 0x0D rd/wr  of diagnostic start addr  (in bytes - absolute value see NIOS memory map)
% buf(15) -- 0x0E rd/wr  of diagnostic data length (in bytes)
% buf(16) -- 0x0F not used

buf=zeros(16,1);
if     flag==0 % normal write
   buf(8) = lastFin * hex2dec('01000000') + firstFin * hex2dec('00010000') + speedTrans * hex2dec('00000100') + 2;
elseif flag==1 % write with update command
   buf(8) = lastFin * hex2dec('01000000') + firstFin * hex2dec('00010000') + speedTrans * hex2dec('00000100') + 1;
end
buf(9) = startAddress;
buf(10)= len * 4;
...
mgp_op_wrsame_sdram(255,255,buf(13)/4,bitand(buf(12),hex2dec('07FFFFFF'))/4,data,connectionNr,dataType);
...
while 1
   try
      ...
      if ~timeout
      %    M4FC_interface_status;
         error('timeout error in write command');
      end
      break
   catch
      mgp_op_wr_m4fci(255,255,6,1,ones(6,1));
%       disp('M4DP write communication error');
      fprintf('$');
   end
end
