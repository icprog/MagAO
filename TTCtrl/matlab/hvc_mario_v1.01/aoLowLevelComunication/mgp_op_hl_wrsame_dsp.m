function mgp_op_hl_wrsame_dsp(firstDsp,lastDsp,len,startAddress,data,varargin)
% mgp_op_hl_wrsame_dsp(firstDsp,lastDsp,len,startAddress,data,[connectionNr],[dataType],[FLAGS])
...
if len>=MAX_DWORD_LEN
    for i=1:floor(len/MAX_DWORD_LEN)
        ...
        writeDsp(firstDsp,lastDsp,MAX_DWORD_LEN,startAddress+(i-1)*MAX_DWORD_LEN,dataBlock,connectionNr,dataType,FLAGS);
    end;
end;
lenLeft=mod(len,MAX_DWORD_LEN);
dataBlock=data((i)*MAX_DWORD_LEN+1:len);
writeDsp(firstDsp,lastDsp,lenLeft,startAddress+i*MAX_DWORD_LEN,dataBlock,connectionNr,dataType,FLAGS);
