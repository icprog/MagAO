...
% x=writeChannelVar(arrayIndex,startAddress,dataType,[dspConfCol],[len],[device],[step_ptr])
...
% 10/05/11 modified to accept any kind of vector order in case of scalar variables
...
if ~isscalar(dataIndex) && isvector(data)
   data=data(:);
end
data=data(dataIndex,:);
CONF_MAT1=CONF_MAT;
...
%    CONF_MAT1=sortrows(CONF_MAT,MY_INDEX); Roby wrong
   ...
   for i=arrayIndex(:)'
      ...
      mgp_op_wrsame_dsp(firstDsp,lastDsp,len,startAddress+offset*step_ptr,dataValue,connectionNr,dataType);
   end
...
elseif strcmpi(device,'FPGA_CNTLOOP') || strcmpi(device,'FPGA_COMPINT')
   CONF_MAT1=uint32(CONF_MAT1);
   if isa(data,'float') && strcmpi(dataType,'single')
      data=single(data);
   end
   if (isa(data,'float') && strcmpi(dataType,'single')) || (isa(data,'int32') && strcmpi(dataType,'int32')) || (isa(data,'uint32') && strcmpi(dataType,'uint32'))
      aa=uint32(zeros(size(data)));
      for ii=1:size(data,2)
         aa(:,ii)=typecast(data(:,ii),'uint32');
      end
      data=aa;
   else
      error('Invalid data class, should be ''%s''',dataType);
   end
   CONF_MAT1(arrayIndex,end+1:end+len)=data;
   if length(arrayIndex)~=size(data,1)
      error('arrayIndex len must be equal to dataLen rows')
   ...
   for i=arrayIndex(:)'
      ...
      dataValue=CONF_MAT1(i,end-len+1:end);
      offset=CONF_MAT1(i,4);
      [aa,bb]=mgp_op_tcp_command('MGP_MEM_WRITE',device,len,startAddress+offset*step_ptr,dataValue,connectionNr);
