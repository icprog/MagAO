...
#ifdef RTDB
// added to use with MsgD-RTDB
      if (doMsgD_RTDB) {
        double VDouble;
        int stat;
        char VarName[VAR_NAME_LEN+1], Time[33];
        MsgBuf msgb;

        snprintf(VarName, VAR_NAME_LEN+1, "%s.%s", MyHostname, label);
        stat = CreaVar (MyName, VarName, REAL_VARIABLE, "G", 1, NULL);
        if (stat != NO_ERROR) {
          sensorLog (LOG_ERR, "MsgD-RTDB: Error creating variable: %d\n",stat);
          ret = 24;
        }
        VDouble = value;
        stat = SetVar(MyName, VarName, REAL_VARIABLE, 1, &VDouble, NULL);
        if(stat != NO_ERROR) {
          sensorLog (LOG_ERR, "MsgD-RTDB: Error setting variable value: %d\n", stat);
          ret = 24;
        }
      }
#endif
