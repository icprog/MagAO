;+
;WRITE_ASCII_STRUCTURE, name_struct, FILENAME=filename
;
;PURPOSE:
...
;
;-
pro write_ascii_structure, name_struct, FILENAME=filename
...
if dim_size[0] eq 0  then begin
    if test_type(struct.(i), /STRING) then printf, unit, name, t_name, " ",struct.(i), FORMAT="(A, A, A,"+frmt_tp+")" else $
