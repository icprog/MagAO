function varargout = aoBufferStop(selection,varargin)
% aoBufferStop(selection,[firstDsp],[lastDsp],[connectionNr])
% selection=array of buffer numbers to stop
...
% mask((selection)*2)=1;
% maskString=num2str(mask(end:-1:1))
% stopData=bin2dec(strrep(maskString,' ',''))
% mgp_op_wrsame_diagbuf(fisrtDsp,lastDsp,1,1,uint32(stopData),'uint32');
