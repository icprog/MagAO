...
% mgp_op_reset_devices(first,last,data,[connectionNr],[dataType])
...
if(length(data) ~= len), error('wrong data length'); end;
...
data=reset_devices(firstDsp,lastDsp,lenLeft,startAddress+i*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);





function data=reset_devices(firstDsp,lastDsp,len,startAddress,data,connectionNr,dataType)
global COMMAND_ID
MGP_DEFINE();
