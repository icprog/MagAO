function convert_xml, filename, root_dir, NAME_LUT=name_lut, PREFIX_LEN=prefix_len
...
function convert_dsp_xml_map, file, FORCE=force, TEXT=text, CRATE=crate, switch_b=switch_b
;$Id: convert_dsp_xml_map.pro,v 1.1 2007/02/14 14:09:10 marco Exp $$
;+
;   NAME:
;    CONVERT_DSP_XML_MAP
;
...
;-
    ...
    case bool_kw of
        ...
        2: begin
            filename_lut = filepath(ROOT=root_dir,"bcu_map_name_lut.txt")
            filename_xml = (file_info(filepath(ROOT=root_dir,"cut_CrateBCU.map.xml")))[0]
            map_filename = file_info(filepath(ROOT=root_dir, "bcu_dm_map.txt"))
            file2add = file_info(filepath(ROOT=root_dir, "bcu_map2add.txt"))
            prefix_len=4
        end

        1: begin
            filename_lut = filepath(ROOT=root_dir,"switch_map_name_lut.txt")
            filename_xml = (file_info(filepath(ROOT=root_dir,"cut_SwitchBCU.map.xml")))[0]
            map_filename = file_info(filepath(ROOT=root_dir, "switch_dm_map.txt"))
            file2add = file_info(filepath(ROOT=root_dir, "switch_map2add.txt"))
            prefix_len=4
        end

        0: begin
            filename_lut = filepath(ROOT=root_dir,"dsp_map_name_lut.txt")
            filename_xml = (file_info(filepath(ROOT=root_dir,"cut_DspMainProgram.map.xml")))[0]
            map_filename = file_info(filepath(ROOT=root_dir, "dsp_dm_map.txt"))
            file2add = file_info(filepath(ROOT=root_dir, "dsp_map2add.txt"))
            prefix_len=3
        end
    ...
    dsp_map = convert_xml(filename_xml.name, NAME_LUT=name_lut, PREFIX_LEN=prefix_len) 
    ...
    for i=0,n_names-1 do begin
        ...
        padded_names[i] = name + string(dsp_map.(i).a, FORMAT="('long x',Z8.8)")
