...
% set zero position
aoWrite('_hvcfc_TT0_pos_command',zeros(sys_data.hvc_nAct,1),0:sys_data.hvc_nAct-1);
aoWrite('_hvcfc_TT0_curr_command',zeros(sys_data.hvc_nAct,1),0:sys_data.hvc_nAct-1);
if 0
   aoBufferArray(1).bufferName='_hvcfc_TT0_float_DAC_value';	            % name of buffer, is not mandatory, just useful
   ...
   aoBufferArray(1).dspPointer=aoGetAddress('_hvcfc_TT0_float_DAC_value'); % pointer of read/write element
   ...
   aoWrite('_hvcfc_TT0_bias_current',sys_data.bias_current_TT,0:sys_data.hvc_nAct-1);      % offset to 35V
...
aoBufferArray(1).bufferName='_hvcfc_TT0_distance';	              % name of buffer, is not mandatory, just useful
aoBufferArray(1).triggerPointer=0;		                          % pointer of trigger
aoBufferArray(1).triggerDataType=3;		                          % trigger data type 1='single' 2='int32' 3='uint32'
aoBufferArray(1).triggerDsp=0;			                          % number of DSP 0 or 1
aoBufferArray(1).triggerMask=4294967295;                         % trigger mask (these bits are checked) normaly should be 0xFFFFFFFF
aoBufferArray(1).triggerValue=uint32(sys_data.hvcCntFreq*.04);   % trigger value
aoBufferArray(1).triggerCompare=0;		                          % trigger compare type: 0: none 1: "value<*pointer" 2: "value>*pointer" 3: "value=*pointer"
aoBufferArray(1).dspPointer=aoGetAddress('_hvcfc_TT0_distance'); % pointer of read/write element
...
aoWrite('_hvcfc_TT0_bias_command',[x_pos y_pos 0 0],0:sys_data.hvc_nAct-1);
aoWrite('_hvcuc_TT0_control_enable',[1 1],0:1);
aoWrite('_hvcfc_TT0_post_loop_gain',sys_data.post_loop_gain,0:sys_data.hvc_nAct-1);
pause(.1);
aoWrite('_hvcfc_TT0_pre_loop_gain',sys_data.pre_loop_gain,0:sys_data.hvc_nAct-1);
pause(.5);
if 0
   aoBufferWriteSetup(aoBufferArray(1));
   aoBufferStart(1,sys_data.hvc_board);
   aoWrite('_hvcfc_TT0_bias_command',sys_data.bias_command_TT,0:sys_data.hvc_nAct-1);
   aoBufferWaitStop(1,sys_data.hvc_board);
   xy_pos=aoBufferReadData(aoBufferArray(1));
   xy_pos=reshape(xy_pos,2,buf_len)';
   x_pos=mean(xy_pos(:,1));
...
% abilitazione della time history ad ampiezza nulla
aoWrite('_hvcfb_CurrHistAmplitude',0,sys_data.hvc_board,sys_data.SCConnectionID);
aoWrite('_hvcub_TT0_CurrHistEnable',1,sys_data.hvc_board,sys_data.SCConnectionID);
