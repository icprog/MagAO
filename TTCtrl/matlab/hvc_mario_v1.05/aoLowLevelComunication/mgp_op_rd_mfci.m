function data=mgp_op_rd_mfci(firstDsp,lastDsp,len,startAddress,varargin)
% mgp_op_rd_mfci(firstDsp,lastDsp,len,startAddress,[connectionNr],[dataType])
...
if startAddress+len>16
    error('wrong start address vs length, should be lower than 16');
end;

udpSocket=getSocket(connectionNr);
data=pnet(udpSocket,'READMGPPACKET',firstDsp,lastDsp,startAddress,161,1,len,dataType,'intel')';
data=double(data);
