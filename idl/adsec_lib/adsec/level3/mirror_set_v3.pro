;$Id: mirror_set_v3.pro,v 1.3 2007/09/19 10:41:14 labot Exp $$
;+
;	NAME:
;		MIRROR_SET_V3
;	
...
;	, STOP_ISOSTATIC = stop_isostatic, SKIP_ASK4CL=skip_ask4cl, OTHER_CL_INIT_POS=other_cl_init_pos)
;
; 	INPUTS:
;
;	OUTPUTS:
...
;   INITIAL_THREE_POS:  position in which close the loop for the three_acts
;   I_GAIN:     initial proportional gain for the 3 closed loop actuators
;   S_GAIN:     minimum proportional gain for the 3 closed loop actuators
;   F_GAIN:     final proportional gain for the 3 closed loop actuators
;   OTHER_CL_INIT_POS: initial delta position command of all other actuators
;
;   COMMON BLOCKS:
...
function mirror_set_v3, INIT_THREE_POS=initial_three_pos, THREE_POS=three_pos, THREE_ACT=three_act $
                   , I_GAIN=i_gain, F_GAIN=f_gain, S_GAIN=s_gain $
...
if n_elements(s_gain) eq 0 then begin           ;low if not defined, just for safety ;;long
    message,'Start gain for isostatic configuration are not defined.', CONT=(sc.debug eq 0)
	return, adsec_error.input_type
endif
...
err = set_isostatic_v3(three_act, initial_three_pos, three_pos, s_gain, i_gain, XADSEC=xadsec, OTHER_CL_INIT_POS=other_cl_init_pos)
if err ne adsec_error.ok then return, err
...
;temp_sys = sys_status
;thr = max(temp_sys[three_act].max_pos-temp_sys[three_act].min_pos)
...
;print, "... ramp the gain for all cl_act loop..."
;
