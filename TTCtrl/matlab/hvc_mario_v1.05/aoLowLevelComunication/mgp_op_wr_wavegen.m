function mgp_op_wr_wavegen(firstDsp,lastDsp,len,startAddress,data,varargin)
% mgp_op_wrsiggen(first,last,len,startAddress,data,[connectionNr],[dataType])
...
if len>=MAX_DWORD_LEN
    for i=1:floor(len/MAX_DWORD_LEN)
        ...
        writeWaveGen(firstDsp,lastDsp,MAX_DWORD_LEN,startAddress+(i-1)*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);
    end;
end;
lenLeft=mod(len,MAX_DWORD_LEN);
dataBlock=data((i)*MAX_DWORD_LEN+1:len);
writeWaveGen(firstDsp,lastDsp,lenLeft,startAddress+i*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);





function data=writeWaveGen(firstDsp,lastDsp,len,startAddress,data,connectionNr,dataType)
global COMMAND_ID
MGP_DEFINE();
...
COMMAND=137; % MGP_OP_WRITE_WAVEGEN_RAM
FLAGS=1;
