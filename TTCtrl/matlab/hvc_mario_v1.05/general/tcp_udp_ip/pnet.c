...
%   linked to MATLAB(R) or any compiled stand alone application.
  ...
  When using Visual C++, compile this with (both 32 & 64 bit OS):
...
#define DEFAULT_WRITETIMEOUT  60 /*sec. double_inf*/
#define DEFAULT_READTIMEOUT   60 /*sec. double_inf*/
...
#define MGP_HEADER_SIZE 14
#define MAX_DWORD_LEN 364
#define MGP_OP_CMD_SUCCESS 200
...
void Print_Start_Message(){
    mexPrintf("\n===============================================================================\n"
          ...
          "Adjusted for Microgate MGP protocol  by Microgate s.r.l\n\n"
	      "===============================================================================\n\n"
	      );
}
...
mxClassID str2classid(const char *str)
{
    ...
    if(myoptstrcmp("INT64",str)==0)     return mxINT64_CLASS;
    if(myoptstrcmp("UINT64",str)==0)    return mxUINT64_CLASS;
...
mxClassID classid2size(const mxClassID id)
{
    ...
    if(id==mxINT64_CLASS)   return 8;
    if(id==mxUINT64_CLASS)  return 8;
...
/********************************************************************/
/* Copys a matlab Char array to a char * string buffer              */
int mgp_mexInputArray2Buff(const int argno,io_buff *buff)
{
    mxClassID id=mxGetClassID(my_mexInputArg(argno));
    int si=classid2size(id);
    int len=mxGetNumberOfElements(my_mexInputArg(argno));
...
void mgp_mexReturnArrayFromBuff(const int maxelements,const int argno,io_buff *buff,const int line)
{
...
/********************************************************************/
/* Write MGP buff input buffer with specified length from UDP or TCP network

int writeMGPbuff(const int firstDsp, const int lastDsp, const int startAddress, int command, int flag, const int len)*/
...
int writeMGPbuff()
{
    ...
    unsigned char mgpHeader[14];
    int cmdId=0,i;
    int reqBufLen;
    int len,startAddress,flags,command,lastDsp,firstDsp;
    int noblock=0,readLen,leftLen,blockLen;
	 int currentPosRead,currentPosWrite; 
    io_buff *sendBuf, *recvBuf;
 
   mxClassID id=mxGetClassID(my_mexInputArg(8));
   int si=classid2size(id);
   int swap=2;
   char *ptr;

     firstDsp=my_mexInputScalar(2);
    lastDsp=my_mexInputScalar(3);
    startAddress=my_mexInputScalar(4);
    command=my_mexInputScalar(5);
    flags=my_mexInputScalar(6);
    len=my_mexInputScalar(7);
    ...
    sendBuf=&con[con_index].write;
    reqBufLen=(MAX_DWORD_LEN*4+MGP_HEADER_SIZE);

    /* Resize readbuffer to needed size */
    if(con[con_index].read.len<MGP_HEADER_SIZE)
	newbuffsize(&con[con_index].read,MGP_HEADER_SIZE);
   
    if(my_mexFindInputOption(9,"NATIVE"))  swap=0;
    if(my_mexFindInputOption(9,"SWAP"))    swap=1;
    if(my_mexFindInputOption(9,"NETWORK")) swap=2;
    if(my_mexFindInputOption(9,"INTEL"))   swap=3;
    ptr = (char *)mxGetPr(gprhs[8]);
    leftLen=len;
    currentPosWrite=con[con_index].write.pos;

    while(leftLen>0)
      {
        /* Resize writebuffer to needed size */
	if(sendBuf->len<reqBufLen)
  		  newbuffsize(sendBuf,reqBufLen);
        con[con_index].write.pos=currentPosWrite;
        blockLen=leftLen<MAX_DWORD_LEN?leftLen:MAX_DWORD_LEN;
        leftLen=leftLen-blockLen;
        cmdId=(cmdId+1)&0xFF;
	dword[0]=(command<<24)|(lastDsp<<16)|(firstDsp<<4);
	dword[1]=blockLen|(flags<<16)|(cmdId<<24);
	dword[2]=startAddress;
	byteswapcopy(&sendBuf->ptr[sendBuf->pos+2],(char *)dword,3,4,3);
	sendBuf->pos+=MGP_HEADER_SIZE;
      	byteswapcopy(&sendBuf->ptr[sendBuf->pos],ptr,blockLen,si,swap);
	sendBuf->pos+=(blockLen*si);
	writedata();
        ptr+=blockLen*si;
        startAddress=startAddress+blockLen;
	/*header loop*/
	currentPosRead=con[con_index].read.pos;
	readLen=0;
	while(1){
	        
	        /*read header*/
		retval=recv(con[con_index].fid,&con[con_index].read.ptr[con[con_index].read.pos],MGP_HEADER_SIZE,MSG_NOSIGNAL);
		if( retval==0){
			mexPrintf("\nREMOTE HOST DISCONNECTED\n");
			con[con_index].status=STATUS_NOCONNECT;
			break;
		...
		*/       ) {
		...
		if( con[con_index].read.pos-currentPosRead>=MGP_HEADER_SIZE)
		break;
	...
	/*test header*/
   		if ((unsigned char)con[con_index].read.ptr[currentPosRead+5]!=MGP_OP_CMD_SUCCESS){
			mexPrintf("Not MGP_OP_CMD_SUCCESS\n");
			perror("Not MGP_OP_CMD_SUCCESS\n");
			break;
		}
		if ((unsigned char)con[con_index].read.ptr[currentPosRead+9]!=cmdId){
		        printf("Wrong cmdId %x %x\n",cmdId,con[con_index].read.ptr[currentPosRead+9]);
			mexPrintf("Wrong cmdId \n");
			perror("Wrong cmdId \n");
			break;
		}
   
	/*remove header*/
        con[con_index].read.pos-=MGP_HEADER_SIZE;
 	}
...
/********************************************************************/
/* Read MGP buff input buffer with specified length from UDP or TCP network

int readMGPbuff(const int firstDsp, const int lastDsp, const int startAddress, int command, int flag, const int len)*/
...
int readMGPbuff()
{
   ...
   /*send mgp packet*/
    ...
    newbuffsize(sendBuf,MGP_HEADER_SIZE);
    /*prepare header dummy words are not touched*/
    /* Resize readbuffer to needed size */
    reqBufLen=(len*4+MGP_HEADER_SIZE);
    if(con[con_index].read.len<reqBufLen)
	newbuffsize(&con[con_index].read,reqBufLen);
	...
	writedata();
	/*header loop*/
	currentPos=con[con_index].read.pos;
	readLen=0;
	while(1){
	        
	        /*read header*/
		retval=recv(con[con_index].fid,&con[con_index].read.ptr[con[con_index].read.pos],MGP_HEADER_SIZE+blockLen*4,MSG_NOSIGNAL);
		if( retval==0){
			mexPrintf("\nREMOTE HOST DISCONNECTED\n");
			con[con_index].status=STATUS_NOCONNECT;
			break;
		...
		*/       ) {
		...
		readLen=retval>0?retval:0;
		con[con_index].read.pos+=readLen;
		if( con[con_index].read.pos-currentPos>=MGP_HEADER_SIZE+blockLen*4)
		break;
		if(timeoutat<=my_now() || noblock)
		break;
	}
	...
	/*test header*/
   		if ((unsigned char)con[con_index].read.ptr[currentPos+5]!=MGP_OP_CMD_SUCCESS){
			mexPrintf("Not MGP_OP_CMD_SUCCESS\n");
			perror("Not MGP_OP_CMD_SUCCESS\n");
			break;
		}
		if ((unsigned char)con[con_index].read.ptr[currentPos+9]!=cmdId){
		        printf("Wrong cmdId %x %x\n",cmdId,con[con_index].read.ptr[currentPos+9]);
			mexPrintf("Wrong cmdId \n");
			perror("Wrong cmdId \n");
			break;
		}
	/*remove header*/
	memmove(&con[con_index].read.ptr[currentPos],&con[con_index].read.ptr[currentPos+MGP_HEADER_SIZE],blockLen*4);
        con[con_index].read.pos-=MGP_HEADER_SIZE;
        startAddress=startAddress+blockLen;
 	}
     mgp_mexReturnArrayFromBuff(len*4,7,&con[con_index].read,0);
}
...
/*	mexPrintf("DEBUG MEX(1):[%d] %s\n",con_index,fun);    DEBUG
 ...
 /*   mexPrintf("DEBUG MEX(2):[%d] %s\n",con_index,fun);    DEBUG*/
    ...
    if(myoptstrcmp(fun,"READMGPPACKET")==0){
	    con[con_index].read.pos=0;
	    readMGPbuff();
	    return;
    }
    if(myoptstrcmp(fun,"WRITEMGPPACKET")==0){
	    writeMGPbuff();
	    return;
    }
