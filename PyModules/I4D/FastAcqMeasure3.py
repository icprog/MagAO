...
_folder_root = Constants.I4D_RAW_DATA_PATH
_raw_folder = Constants.I4D_RAW_DATA_PATH
_calibrated_folder = Constants.I4D_CALIB_DATA_PATH
_pack = 1
...
_fs = Store.FetchKey('hardware.inventory.framesource')

# Hw trigger
_trigger = True#modified 12 july 2009 Runa Briguglio
...
_fmr = _fs.GetFactoryRect()
if _fmr:
	fmr_l, fmr_t, fmr_w, fmr_h = _fmr # unpack
else:
...
a=('x0: %d, y0: %d, dx: %d, dy: %d ' % (_x0, _y0, _dx, _dy))
print 'frame info'
print a
...
### PYRO LOGGING ###
#import Pyro.util
#logSource = "CONTROLLER I4D"
#userLog = Pyro.util.UserLogger()
#def log(message):
#	   userLog.msg(logSource, message)


#
# Acquire data form camera and stream to disk
#
def __acquire(nMeas, burstFolder, fileNamePrefix):
	
	fullPath = Constant4d.imgdir +'/'+ burstFolder
	fullRawPath = fullPath + '/' + _raw_folder
	...
	xpix_val = GetValue(_fs.GetPixelSpacing())
	xpix_unit = GetUnit(_fs.GetPixelSpacing())
	maxint = _fs.GetMaxInt()
	wedge = Store.FetchFloat('hardware.wedge')
	wavelength_val = GetValue(_fs.wavelength)
	wavelength_unit = GetUnit(_fs.wavelength)
	...
	tempo0=time.clock()
	_fs.PreGrab(hint=None)
	
	# Collect and save a number of frames
	#log("Doing IFC_ext.StreamToDisk...")
	...
	if rep < tout:
		...
		print 'No trigger detected, acquisition terminated'
		raise

	_fs.PostGrab()

	# the lines below were not commented
	...
	#print camInfo    !!!was not commented!!!
	#nCollected = gc.collect()
	#print 'Frequency estimation : '+freq_info
...
def __calibrate(burstFolder):
	...
	fullPath = Constant4d.datadir +'/'+ burstFolder #***modified to calibrate directly into final directory	#fullPath = 'Z:/4D/' + burstFolder
	print Constant4d.datadir
	print fullPath
	fullRawPath = fullPath + '/raw'# + _raw_folder
	fullCalibPath = fullPath+ '/calibrated'# + _calibrated_folder  #***modified to calibrate directly into final directory
	#fullCalibPath ='D:/Zcopy'+'/'+burstFolder
	#os.mkdir(fullCalibPath)
	#fullCalibPath =fullCalibPath+ '/calibrated'
	#os.mkdir(fullCalibPath)
	# Note that 'burstFolder' exists, created by __acquire
	if not os.path.exists(fullRawPath):
		return
	...
	for input_filename in input_filenames:
		...
		# Apply calibrations
		_fs.SetFullFrame(full_array)
		_fs.ApplyCorrections()
		#_fs.ApplyFactoryRect() we're restricting to _fmr in acq
		_fs.ApplySampling()
		full_array = _fs.GetFullFrame()
		red = _fs.GetReduction() or 1
		print ('red=',red)
		...
		#log("Doing FastAcq.WriteRawToDisk...")
		...
		#log("FastAcq.WriteRawToDisk done!")
		
	# Remove raw files
	if Constants.I4D_REMOVE_RAW_FRAMES:
		raw_filenames = os.listdir(fullRawPath)
		for file in raw_filenames:
			os.remove(fullRawPath + '/' + file)
			os.rmdir(fullRawPath)
...
# If n = 1, do nothing
#
def __sample(n, burstFolder):
	if n<=1:
		return # n must be greater than 1
	
	fullRawPath = _folder_root + '/' + burstFolder + '/' + _raw_folder
	fullCalibPath = _folder_root + '/' + burstFolder + '/' + _calibrated_folder
	...
	for i in range(len(input_filenames)):
		if i%n:
			input_fullpath = os.path.join(fullRawPath, input_filenames[i])
			os.remove(input_fullpath)
	
	
### MAIN METHODS ###

def setTrigger(enable):
	_fs.SetTriggeredMode(enable) # Doesn't returns anything!
	

def acquire(step,nMeasures, burstFolder, fileNamePrefix):
	imdir=Constant4d.imgdir+burstFolder+'/raw/'
	#fout='D:/write-test.out'
	#file2rem=os.listdir(imdir)
	#for i in range(len(file2rem)):
	#	os.remove(imdir+file2rem[i])
	...
	if step == 0:
		try:
			tempo0=time.clock()
			(frameInfo, camInfo) = __acquire(nMeasures, burstFolder, fileNamePrefix)
			tempo1=time.clock()
			freq=nMeasures/(tempo1-tempo0)
		
		
			#__sample(_meas_sample, burstFolder)  
