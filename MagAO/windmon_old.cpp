...
{   
   ...
   _windsLogger = Logger::get("WINDS", Logger::LOG_LEV_INFO, "TELEMETRY");
...
void windmon::DoFSM()
{
   ...
   //var_ambwind.Set(wxwind, 0, FORCE_SEND);
   ...
   //std::cout << "airspeed = " << airspeed  << " (m/s)\twxwind  = " << wxwind << " (m/s)\n";
...
int windmon::TestNetwork(void)
{
   ...
   if(meas.length() > 6)
   {
      measures[avg_idx] = strtod(meas.substr(4,4).c_str(),0);

   
      if(meas.find("FPM",0) != (unsigned) -1) measures[avg_idx] *= (.3048/60.);
