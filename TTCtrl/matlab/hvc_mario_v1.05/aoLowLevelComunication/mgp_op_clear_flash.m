function mgp_op_clear_flash(firstDsp,lastDsp,len,startAddress,data,varargin)
% mgp_op_clear_flash(firstDsp,lastDsp,len,startAddress,data,[connectionNr],[dataType])
...
if len>=MAX_DWORD_LEN
    for i=1:floor(len/MAX_DWORD_LEN)
        ...
        writeSdram(firstDsp,lastDsp,MAX_DWORD_LEN,startAddress+(i-1)*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);
    end;
end;
lenLeft=mod(len,MAX_DWORD_LEN);
dataBlock=data((i)*MAX_DWORD_LEN+1:len);
writeSdram(firstDsp,lastDsp,lenLeft,startAddress+i*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);





function data=writeSdram(firstDsp,lastDsp,len,startAddress,data,connectionNr,dataType)
global COMMAND_ID
MGP_DEFINE();
...
pnet(udpSocket,'setreadtimeout',MGP_READTIMEOUT_LONG);
...
pnet(udpSocket,'setreadtimeout',MGP_READTIMEOUT_SHORT);
