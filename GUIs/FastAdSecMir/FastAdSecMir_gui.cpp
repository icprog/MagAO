...
//    tab1Layout->addWidget( mir[1] );
    ...
    fplot = new QwtPlot( QwtText("Peak mirror force (N)"), this);
    fplot->setGeometry( QRect( 300, 120, 500, 220 ) );
    fplot->setAxisScale(QwtPlot::yLeft, 0, 0.8 );

    c = new QwtPlotCurve("0");
    c->attach(fplot);

    for (int i=0; i<NDATA; i++)
       x[i] = i;
    memset( y, 0, sizeof(double)*NDATA);
...
/*
    ...
    int stat;
    if((stat=pthread_create(&license_thread,&attribute,GetThread, (void*)this))!=0) {
        _logger->log(Logger::LOG_LEV_ERROR, "Error: cannot start license check thread (%s)", strerror(stat));
    }
*/
}

void *GUIadSecM::GetThread( void *gui) {

   return NULL;

/// DISABLED

   GUIadSecM *self = (GUIadSecM*)gui;
...
void GUIadSecM::GetAll()
{
    ...
    ParamDict pdict;
    ValueDict dict, fastdict, housedict, fastdict2;
...
//qWarning( "GUIadSecM::GetAll(): entered" );
    ...
    try{
         DiagnWhich which("CHNEWDELTACOMMAND", from, to);
         fastdict = GetDiagnValue( (char *)diagnapp.c_str(),which, timeout);

         DiagnWhich which2("CHCURRAVERAGE", from, to);
         fastdict2 = GetDiagnValue( (char *)diagnapp.c_str(),which2, timeout);

         //fastdict.insert( fastdict2.begin(), fastdict2.end());
    ...
    float fmin=1e9;
    float fmax=-1e9;
    for (ValueDict::iterator it = fastdict2.begin(); it != fastdict2.end();  it++){
             DiagnWhich wh = (*it).first;
             DiagnValue va = (*it).second;

             float v = va.Last();
             if (fabs(v)>fmax) fmax= fabs(v);
             if (fabs(v)<fmin) fmin = fabs(v);
    }

    memmove( y+1, y, sizeof(double)*(NDATA-1));
    y[0] = fmax;
...
/*
    ...
    try{
         DiagnWhich which("Switch*", from, to);
         housedict = GetDiagnValue( (char *)diagnapp1.c_str(),which, timeout);
        } catch (AOException &e) {
                 if ( timer->isActive() ) timer->stop();
                 buttonStart->setText( tr( "&Start Refresh" ) );
                 buttonStart->setPaletteBackgroundColor( QColor( 200, 0, 0 ) );
    ...
    printf("Got %d variables from housekeeper\n", housedict.size());
...
*/
        ...
        for (ValueDict::iterator it = fastdict.begin(); it != fastdict.end();  it++){
///////printf("GETALL-----it= %i\n",jj);
             DiagnWhich wh = (*it).first;
             DiagnValue va = (*it).second;
...
//printf ("GetAll 10a size=%d\n",dict.size());
        jj=-1;
        //Iterate over the dictionary of the retrieved DiagnVars.
        for (ValueDict::iterator it = housedict.begin(); it != housedict.end();  it++){
             DiagnWhich wh = (*it).first;
             DiagnValue va = (*it).second;
             ...
             string fam = family;
             for (int i=0; i<fam.size(); i++) fam[i] = toupper(fam[i]);

             if (Family != fam) continue;
