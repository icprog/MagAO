pro xdds_gui_draw_rings, ima, act_color, SMOOTH=smooth, LIST=list

    @adsec_common
	common xdds_gui_block, frame, slope, dm, xdds_data
	common xdds_gui_draw_block, capsens
    ...
    if keyword_set(smooth) then begin
        ...
        if n_elements(idx) eq 0 then begin
            ...
            x_in = xdds_data.act_coord[0,*]*(1.0+1e-4*randomn(seed,adsec.n_actuators))
            y_in = xdds_data.act_coord[1,*]*(1.0+1e-4*randomn(seed,adsec.n_actuators))
        endif
    	...
    	act_coord=transpose(xdds_data.act_coord)/(adsec_shell.out_radius*2)+rebin([0.5,0.5],adsec.n_actuators,2)
    	s = size(ima)
