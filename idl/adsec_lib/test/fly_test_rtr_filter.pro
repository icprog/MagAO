;Script to test the m calcolo, e calcolo and d calcolo con cambiamento al volo del blocco di memoria
;pulizia G,b0,Bi,Ai
block = 0
test  = 4
...
;f- G=0, a1=0,e controllo che rimanga cosi' 
        ...
        ; setto entrambi i g a 0
        err = set_g_gain(sc.all, g_t00, /block)
        err = set_g_gain(sc.all, g_t00)
        Sf_t00 = randomn(seed,rtr.n_slope)

        ;annullo Ai
        ad_t00 = fltarr(adsec.n_actuators, adsec.n_actuators, rtr.n_slope_delay)
...
;   start rtr e verifica che il modo sia corretto
    1: begin
            ...
            ;setto entrambi i blocchi di g a 1
            err = set_g_gain(sc.all, fltarr(adsec.n_actuators)+1, /block)
            err = set_g_gain(sc.all, fltarr(adsec.n_actuators)+1)
            for pp=0,10 do begin
                ...
                err = write_same_dsp(sc.all,rtr_map.slope_vector, sf_t01)
                                ;annullo Ai
                ad_t01 = fltarr(adsec.n_actuators, adsec.n_actuators, rtr.n_slope_delay)
                err = set_a_delay_matrix(ad_t01)
                                ;annullo Bi
                bd_t01 = fltarr(rtr.n_slope, adsec.n_actuators, rtr.n_slope_delay)
                err = set_b_delay_matrix(bd_t01, block=block)
...
;b- idem a sopra ma con guadagni random
        ...
        ;annullo Ai
        ad_t02 = fltarr(adsec.n_actuators, adsec.n_actuators, rtr.n_slope_delay)
        ...
        err = start_ff()
        cc=1
        err = read_seq_dsp(sc.all, rtr_map.start_rtr, 1l, nendof_ff)
        while (total(abs(nendof_ff)) ne 0) or cc eq 10 do begin
            cc +=1
            err = read_seq_dsp(sc.all, rtr_map.start_ff, 1l, nendof_ff)
        endwhile
...
;                err = write_same_dsp(sc.all, dsp_map.modes_delay, fltarr(192))
                ...
                ;    cc +=1
                ;    err = read_seq_dsp(sc.all, rtr_map.nendof_rtr, 1l, nendof_rtr)
                ;endwhile
                ;if cc eq 10 then message, 'Conto modi non ancora finito' else print, 'Conto modi finito in '+strtrim(string(cc),2)+' tentativi'
                print, 'Starting MM #'+strtrim(string(j),2)
                err = start_mm()
...
;e- continuo come d finche' non finisco la linea di ritardo
        ...
        for i=0, rtr.n_modes_delay-1 do begin
            ...
            ad_t04 = fltarr(adsec.n_actuators, adsec.n_actuators, rtr.n_slope_delay)
            ad_t04[*,*,i] = tmp
