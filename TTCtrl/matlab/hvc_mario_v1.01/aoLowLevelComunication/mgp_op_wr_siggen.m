function mgp_op_wrsiggen(firstDsp,lastDsp,len,startAddress,data,varargin)
...
if len>=MAX_DWORD_LEN
    for i=1:floor(len/MAX_DWORD_LEN)
        ...
        writeSigGen(firstDsp,lastDsp,MAX_DWORD_LEN,startAddress+(i-1)*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);
    end;
end;
lenLeft=mod(len,MAX_DWORD_LEN);
dataBlock=data((i)*MAX_DWORD_LEN+1:len);
writeSigGen(firstDsp,lastDsp,lenLeft,startAddress+i*MAX_DWORD_LEN,dataBlock,connectionNr,dataType);
