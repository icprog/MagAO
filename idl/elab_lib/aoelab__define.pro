	...
	if obj_valid(self._tel) then begin
        ...
        if obj_valid(self->wfs_status()) then if strmid((self->wfs_status())->wunit(),0,4) eq 'LBTI' then begin
            ...
            if obj_valid(self->wfs_status()) then if strmid((self->wfs_status())->wunit(),0,3) eq 'MAG' then begin
              ...
              if (self->wfs_status())->lamp_intensity() gt .001 then self._operation_mode = "RR" else self._operation_mode = "ONSKY"
            endif else begin
...
pro AOelab::modalplot, OVERPLOT = OVERPLOT, COLOR=COLOR, clvar=clvar, olvar=olvar, _extra=ex
...
function AOelab::duration
    ...
    return, max([d_irtc, d_luci, d_pisces, d_loop])
end

pro AOelab::psf, PARENT = PARENT, fullframe=fullframe, sr=sr
    loadct,3
    if ((not obj_valid(self->irtc())) and (not obj_valid(self->pisces())) and (not obj_valid(self->luci())) ) then return
