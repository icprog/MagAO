function x=readSramByIndex(dspArray,connectionNr,len,startAddress,dataType)
...
if len>7;
    for i=dspList(:)'
        readData=[readData;mgp_op_rd_sram(i,i,len,startAddress,connectionNr,dataType)];
    end;
else
    if isempty(readDspList)
        firstDsp=dspList(1);
        lastDsp=dspList(end);
        readData=mgp_op_rd_sram(firstDsp,lastDsp,len,startAddress,connectionNr,dataType);
    else
        firstDsp=dspList(1);
        for i=reshape(readDspList,1,length(readDspList))
        lastDsp=dspList(i);    
        readData=[readData;mgp_op_rd_sram(firstDsp,lastDsp,len,startAddress,connectionNr,dataType)];
