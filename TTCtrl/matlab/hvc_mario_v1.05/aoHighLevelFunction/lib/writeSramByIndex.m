function x=writeSramByIndex(dspArray,connectionNr,len,startAddress,data,dataType)
...
if size(unique(dataMat(:,2:end),'rows'),1)==1
    ...
    if isempty(writeDspList)
        firstDsp=dspList(1);
        lastDsp=dspList(end);
        mgp_op_wrsame_sram(firstDsp,lastDsp,len,startAddress,dataMat(1,2:end),connectionNr,dataType);
    else
        firstDsp=dspList(1);
...
[nr,nc]=size(dataMat);
for i=1:nr
    mgp_op_wrsame_sram(dataMat(i,1),dataMat(i,1),len,startAddress,dataMat(i,2:end),connectionNr,dataType);
end;
